...
let makemsg = function (username, msg, time, isleft, issys, ishist) {
    return $(`<div class="ui message ${issys ? 'red' : ''}" align="${isleft ? 'left' : 'right'}" style="max-width: 50%; ${isleft ? '' : 'margin-right: 0; margin-left: 50%;'}">
                    <div class="ui tag label" ${ishist ? '' : 'style="display: none;"'}>留言</div>
    ...
    $(`<div class="ui bottom attached tab segment" data-tab="${'dt' + this.cnt}" id="${'rem2' + this.cnt}">
        ...
        <div class="ui container">
            ...
            <div class="ui bottom attached basic buttons">
                ...
                <button class="ui button" id="sendhistico${this.cnt}"><i class="history icon"></i> 发送离线消息</button>
...
$('#joinsubmit').click(function () {
    ...
    sck.on('new message', function (data) {
        ...
        if(cnt) {
            ...
            $(`#fld${cnt}`)
                .append(makemsg(username, msg, time, username === sck.username ? false : true, false, ishist))
    ...
    sck.on('sys message', function (data) {
        ...
        if(cnt) {
            ...
            $(`#fld${cnt}`)
                .append(makemsg('系统消息', msg, time, true, true, false))
                .scrollTop($(`#fld${cnt}`)[0].scrollHeight);
        }
    });
});
